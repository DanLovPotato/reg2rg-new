...
class PatchEmbeddingBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        dim, nd, nh, nw = x.shape[1:]
        if self.proj_type == "conv":
            x = x.flatten(2).transpose(-1, -2)
        
        if nd == 7 and nh == 16 and nw == 11:
            embeddings = x + self.position_embeddings   
        else:
            pos_embed = self.position_embeddings.permute(0, 2, 1)
            pos_embed = pos_embed.view(1, dim, -1, 16, 11)
            # trilinear interpolation has no BFloat16/Half kernel on this torch version;
            # interpolate in float32 then cast back to match x's dtype
            pos_embed = F.interpolate(pos_embed.float(), size=(nd, nh, nw), mode='trilinear', align_corners=False).to(x.dtype)
